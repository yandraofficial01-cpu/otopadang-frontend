...
function ImageSlider({ item, tipe, theme }) {
  ...
  ].filter(url => url); // buang yg null

  const prev = (e) => { e.stopPropagation(); setCurrent(current === 0? imageUrls.length - 1 : current - 1); };
  const next = (e) => { e.stopPropagation(); setCurrent(current === imageUrls.length - 1? 0 : current + 1); };
      ...
      {imageUrls.length > 1 && (
        <>
          ...
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5">
            {imageUrls.map((_, i) => <div key={i} className={`w-2 h-2 rounded-full transition ${i === current? 'bg-yellow-400' : 'bg-white/50'}`}></div>)}
          </div>
        </>
      )}
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-md ${bgHeader} border-b ${theme === 'dark'? 'border-gray-800' : 'border-gray-200'}`}>
        <div className="container mx-auto max-w-7xl px-4 py-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {navLinks.map(link => (<Link key={link.name} href={link.href} className="hover:text-yellow-400 transition">{link.name}</Link>))}
          </nav>
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className={`md:hidden border-t ${theme === 'dark'? 'border-gray-800' : 'border-gray-200'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {mobil.map(item => (
              <div key={item.id} className={`group rounded-2xl border ${bgCard} overflow-hidden shadow-lg hover:shadow-yellow-500/20 transition`}>
                <ImageSlider item={item} tipe="mobil" theme={theme} />
                <div className="p-5">
                  <h3 className="text-xl font-bold">{item.merek} {item.nama_mobil}</h3>
                  <p className={`text-2xl font-bold text-yellow-400 my-2`}>Rp {item.harga?.toLocaleString('id-ID')}</p>
                  <p className={`${textMuted} text-sm mb-4`}>{item.tahun} • {item.transmisi} • {item.kilometer?.toLocaleString('id-ID')} KM</p>
      ...
      {!loading && rumah.length > 0 && (
        <section className="container mx-auto max-w-7xl px-4 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {rumah.map(item => (
              <div key={item.id} className={`group rounded-2xl border ${bgCard} overflow-hidden shadow-lg hover:shadow-yellow-500/20 transition`}>
                <ImageSlider item={item} tipe="rumah" theme={theme} />
                <div className="p-5">
