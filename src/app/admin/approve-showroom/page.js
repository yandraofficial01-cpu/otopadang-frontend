...
export default function ApproveShowroomPage() {
  ...
  const token = localStorage.getItem("token"); // ganti kalo pake context
  ...
  const fetchShowrooms = async () => {
    ...
    try {
      ...
      console.log(e);
      alert("Gagal ambil data showroom");
    }
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 bg-[#0B0B0F] min-h-screen text-white">
      ...
      {!loading && showrooms.length === 0 && (
        <p className="text-gray-400">Belum ada showroom</p>
      )}
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  <span className={`text-xs px-2 py-1 rounded ${s.status === 'pending' ? 'bg-yellow-600' : 'bg-green-600'}`}>
                    {s.status}
                  </span>
