...
export default function LoginPage() {
  ...
  // TAMBAH INI: Kalau udah login, langsung tendang ke dashboard
  useEffect(() => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      router.replace('/dashboard') // ganti push jadi replace biar ga bisa back
    } catch (err) {
    ...
    <div className="flex justify-center items-center h-screen bg-black">
      <form onSubmit={handleLogin} className="p-8 shadow-lg rounded-lg w-96 bg-gray-900 text-yellow-400">
        ...
        <input
          ...
          className="border-gray-700 p-2 w-full mb-3 rounded bg-black text-white"
          required
        />
        ...
        <input
          ...
          className="border-gray-700 p-2 w-full mb-4 rounded bg-black text-white"
          required
        />
