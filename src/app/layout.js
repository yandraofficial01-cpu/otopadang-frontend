...
import { useState } from "react"; // TAMBAHIN INI
import { Menu, X } from "lucide-react"; // install dulu: npm i lucide-react
...
export default function RootLayout({ children }) {
  const [isOpen, setIsOpen] = useState(false); // STATE BUAT DROP SLIDE
    ...
    <html lang="id" className={`${geistSans.variable} ${geistMono.variable} h-full antialiased`}>
      <body className="min-h-full flex-col bg-[#0B0B0F] text-gray-200">
        ...
        <header className="bg-[#0B0B0F]/80 backdrop-blur-md border-b border-gray-800 sticky top-0 z-50">
          ...
          {/* DROP SLIDE MOBILE */}
          {isOpen && (
            <div className="md:hidden bg-[#0B0B0F] border-t border-gray-800 px-4 py-4 flex flex-col gap-4">
